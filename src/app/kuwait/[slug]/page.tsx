...
import { KUWAITI_CITIES, getKuwaitiCityBySlug, type KuwaitiCity } from '@/data/kuwait-cities';

type Params = { slug: string };

export function generateStaticParams(): Params[] {
  return KUWAITI_CITIES.map((c) => ({ slug: c.slug }));
}

export const dynamicParams = false;

export function generateMetadata({ params }: { params: Params }): Metadata {
  const city = getKuwaitiCityBySlug(params.slug);
  if (!city) return {};
  const pageUrl = `${SITE.url}/kuwait/${city.slug}`;
  const title = `AI Development Company in ${city.name}, Kuwait | Brocode`;
  return {
    title,
    ...
    twitter: {
      ...
      title,
      description: `Production AI for ${city.name}. ${city.region}. Engineering depth in the State of Kuwait, not slideware.`,
      images: ['/images/general/og-default.jpg']
    }
  };
}
...
const serviceTileTemplate = (city: KuwaitiCity) => [
  {
  ...
  },
  {
    ...
    angle: `Contact-centre assistants that survive Kuwaiti-dialect calls in ${city.name}.`,
  ...
  },
  {
    ...
    result: 'Reference architectures for Microsoft Azure Kuwait, Oracle Cloud Kuwait, Zain Cloud and Ooredoo Cloud.'
  },
  {
...
export default function KuwaitCityPage({ params }: { params: Params }) {
  const city = getKuwaitiCityBySlug(params.slug);
  if (!city) notFound();

  const pageSlug = city.slug;
  const pagePath = `/kuwait/${city.slug}`;
  const pageUrl = `${SITE.url}${pagePath}`;

  const whatsappMessage = `Hi Brocode — I'm exploring AI delivery partners for [organisation] in ${city.name}, Kuwait. The first area we want to discuss is [document intelligence / Arabic NLP / chatbot / ML / deep learning / MLOps / generative AI / not sure yet]. Could we schedule a 30-minute call?`;

  const formConfig: PageFormConfig = {
    pageSlug,
    title: `Talk to an expert — ${city.name} desk`,
    subtitle: `One short qualification form. A senior engineer on our Kuwait delivery desk — not an SDR — responds within one business day with a named contact and a proposed shape for the engagement in ${city.name}.`,
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          ...
          'Vision 2035 / New Kuwait programme',
  ...
  const relatedCities = city.relatedCitySlugs
    .map((s) => getKuwaitiCityBySlug(s))
    .filter((c): c is KuwaitiCity => Boolean(c));

  const relatedLinks = [
    {
      label: 'AI development in Kuwait (country overview)',
      href: '/kuwait/ai-development-in-kuwait',
      description: 'The State-wide page — CBK, CMA-Kuwait, CITRA-DPPR, KIAS and the 12-week production path.'
    },
  ...
  return (
    <>
          ...
          __html: JSON.stringify([
            {
              ...
              areaServed: [
                { '@type': 'City', name: city.name, containedInPlace: { '@type': 'Country', name: 'Kuwait', identifier: 'KW' } },
            ...
            },
            {
              ...
              '@id': `${pageUrl}#webpage`,
              url: pageUrl,
              name: `AI Development Company in ${city.name}, Kuwait | Brocode`,
              inLanguage: ['en-KW', 'en-AE', 'ar-KW'],
              isPartOf: { '@id': `${SITE.url}/#website` },
              about: {
                '@type': 'Thing',
                name: `AI development services for enterprises in ${city.name}, ${city.region}, Kuwait`
              },
            ...
            },
            {
              ...
              name: `AI Development in ${city.name}, Kuwait`,
              provider: { '@id': `${SITE.url}/#organization` },
              areaServed: {
                '@type': 'City',
                name: city.name,
                containedInPlace: { '@type': 'Country', name: 'Kuwait', identifier: 'KW' }
              },
              audience: {
            ...
            },
            {
              ...
              itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: 'Kuwait', item: `${SITE.url}/kuwait/ai-development-in-kuwait` },
      ...
      <section className="bg-surface-muted py-22 md:py-30">
        <div className="container-site">
          <SectionHeading
            ...
            description={`Each tile routes to a service page with the named engineers, the reference architecture, a pre-contract benchmark offer, and the KWD rate band for ${city.name} delivery.`}
          />
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`Same model, same gates, same evidence — whether the work in ${city.name} is Arabic OCR for a Kuwaiti ministry, a fraud model for a CBK-supervised bank, or a deep-learning programme on a KOC / KNPC / KIPIC asset.`}
            />
          </div>
      ...
      <section id="buyers-guide" className="bg-white py-22 md:py-30">
        <div className="container-site">
          <LeadMagnetGate
            ...
            contents={[
              `Procurement reality in ${city.name} and Kuwait`,
      ...
      {/* Related Kuwait cities — internal linking */}
        ...
        <section className="border-y border-ink-100 bg-surface-muted py-14">
          <div className="container-site">
            <p className="text-center text-xs font-semibold uppercase tracking-wider text-ink-500">
              Production AI delivered across Kuwait
            </p>
            ...
            <p className="mt-8 text-center text-sm text-ink-500">
              ...
              <Link href="/kuwait/ai-development-in-kuwait" className="text-brand underline">
                AI development in Kuwait
              </Link>
              .
            </p>
          </div>
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`These are the questions our delivery leads field most often from ${city.name} CIO offices, sector-regulated risk teams, and Kuwaiti procurement reviewers.`}
            />
          </div>
          <div className="lg:col-span-7">
            <FAQ items={faqItems} />
