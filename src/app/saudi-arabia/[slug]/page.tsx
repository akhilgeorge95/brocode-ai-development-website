...
import { SAUDI_CITIES, getSaudiCityBySlug, type SaudiCity } from '@/data/saudi-cities';
...
export function generateMetadata({ params }: { params: Params }): Metadata {
  const city = getSaudiCityBySlug(params.slug);
  if (!city) return {};
  const pageUrl = `${SITE.url}/saudi-arabia/${city.slug}`;
  const title = `AI Development Company in ${city.name}, Saudi Arabia | Brocode`;
  return {
    title,
    ...
    twitter: {
      ...
      title,
      description: `Production AI for ${city.name}. ${city.region}. Engineering depth in the Kingdom, not slideware.`,
      images: ['/images/general/og-default.jpg']
    }
  };
}
...
const serviceTileTemplate = (city: SaudiCity) => [
  {
  ...
  },
  {
    ...
    angle: `Contact-centre assistants that survive Saudi-dialect calls in ${city.name}.`,
...
export default function SaudiCityPage({ params }: { params: Params }) {
  const city = getSaudiCityBySlug(params.slug);
  if (!city) notFound();

  const pageSlug = city.slug;
  const pagePath = `/saudi-arabia/${city.slug}`;
  const pageUrl = `${SITE.url}${pagePath}`;

  const whatsappMessage = `Hi Brocode — I'm exploring AI delivery partners for [organisation] in ${city.name}, Saudi Arabia. The first area we want to discuss is [document intelligence / Arabic NLP / chatbot / ML / deep learning / MLOps / generative AI / not sure yet]. Could we schedule a 30-minute call?`;

  const formConfig: PageFormConfig = {
    pageSlug,
    title: `Talk to an expert — ${city.name} desk`,
    subtitle: `One short qualification form. A senior engineer on our Saudi delivery desk — not an SDR — responds within one business day with a named contact and a proposed shape for the engagement in ${city.name}.`,
  ...
  const relatedCities = city.relatedCitySlugs
    .map((s) => getSaudiCityBySlug(s))
    .filter((c): c is SaudiCity => Boolean(c));

  const relatedLinks = [
    {
      label: `AI development in Saudi Arabia (country overview)`,
      href: '/saudi-arabia/ai-development-in-saudi-arabia',
      description: 'The Kingdom-wide page — SAMA, SDAIA, NCA-ECC, PDPL and the 12-week production path.'
    },
  ...
  return (
    <>
          ...
          __html: JSON.stringify([
            {
              ...
              areaServed: [
                { '@type': 'City', name: city.name, containedInPlace: { '@type': 'Country', name: 'Saudi Arabia', identifier: 'SA' } },
            ...
            },
            {
              ...
              '@id': `${pageUrl}#webpage`,
              url: pageUrl,
              name: `AI Development Company in ${city.name}, Saudi Arabia | Brocode`,
              inLanguage: ['en-SA', 'en-AE', 'ar-SA'],
              isPartOf: { '@id': `${SITE.url}/#website` },
              about: {
                '@type': 'Thing',
                name: `AI development services for enterprises in ${city.name}, ${city.region}, Saudi Arabia`
              },
            ...
            },
            {
              ...
              name: `AI Development in ${city.name}, Saudi Arabia`,
              provider: { '@id': `${SITE.url}/#organization` },
              areaServed: {
                '@type': 'City',
                name: city.name,
                containedInPlace: { '@type': 'Country', name: 'Saudi Arabia', identifier: 'SA' }
              },
              audience: {
            ...
            },
            {
              ...
              itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: 'Saudi Arabia', item: `${SITE.url}/saudi-arabia/ai-development-in-saudi-arabia` },
          ...
          <div className="lg:col-span-5">
            <Reveal>
              ...
              <p className="mt-5 max-w-md text-lg leading-relaxed text-ink-600">
                ...
                engineers, a written 12-week production path, and a PDPL- and SAMA-aligned handover pack —
      ...
      <section className="bg-surface-muted py-22 md:py-30">
        <div className="container-site">
          <SectionHeading
            ...
            description={`Each tile routes to a service page with the named engineers, the reference architecture, a pre-contract benchmark offer, and the SAR rate band for ${city.name} delivery.`}
          />
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`Same model, same gates, same evidence — whether the work in ${city.name} is Arabic OCR for a Saudi ministry, a fraud model for a SAMA-supervised bank, or a deep-learning programme on the plant floor.`}
            />
          </div>
      ...
      <section id="buyers-guide" className="bg-white py-22 md:py-30">
        <div className="container-site">
          <LeadMagnetGate
            ...
            contents={[
              `Procurement reality in ${city.name} and Saudi Arabia`,
      ...
      {/* Related Saudi cities — internal linking */}
        ...
        <section className="border-y border-ink-100 bg-surface-muted py-14">
          <div className="container-site">
            <p className="text-center text-xs font-semibold uppercase tracking-wider text-ink-500">
              Production AI delivered across Saudi Arabia
            </p>
            ...
            <p className="mt-8 text-center text-sm text-ink-500">
              ...
              <Link href="/saudi-arabia/ai-development-in-saudi-arabia" className="text-brand underline">
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`These are the questions our delivery leads field most often from ${city.name} CIO offices, sector-regulated risk teams, and Saudi procurement reviewers.`}
            />
          </div>
          <div className="lg:col-span-7">
            <FAQ items={faqItems} />
