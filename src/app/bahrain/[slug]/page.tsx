...
import { BAHRAINI_CITIES, getBahrainiCityBySlug, type BahrainiCity } from '@/data/bahrain-cities';

type Params = { slug: string };

export function generateStaticParams(): Params[] {
  return BAHRAINI_CITIES.map((c) => ({ slug: c.slug }));
}

export const dynamicParams = false;

export function generateMetadata({ params }: { params: Params }): Metadata {
  const city = getBahrainiCityBySlug(params.slug);
  if (!city) return {};
  const pageUrl = `${SITE.url}/bahrain/${city.slug}`;
  const title = `AI Development Company in ${city.name}, Bahrain | Brocode`;
  return {
    title,
    ...
    twitter: {
      ...
      title,
      description: `Production AI for ${city.name}. ${city.region}. Engineering depth in the Kingdom of Bahrain, not slideware.`,
      images: ['/images/general/og-default.jpg']
    }
  };
}
...
const sharedBaseFaqs: FAQItem[] = [
  ...
  {
    question: 'How do you handle Bahrain PDPL, CBB Rulebook model risk and NCSC alignment?',
    answer:
      "Sovereign deployment inside the Kingdom is our default — and Bahrain has a structural advantage here as the home of the first AWS region in the GCC (AWS Middle East / Bahrain), alongside Microsoft Azure, Batelco Cloud and stc Bahrain. Customer-managed keys via Thales or Entrust HSM is standard. Every engagement ships a PDPL DPIA aligned to Law No. 30 of 2018, a CBB Rulebook-style model risk file where the entity is CBB-supervised (covering banking, insurance and capital-markets requirements as relevant), an NCSC controls mapping and an iGA / national-AI cross-walk. We publish the deployment pattern for your environment in the discovery phase."
  },
  {
...
const serviceTileTemplate = (city: BahrainiCity) => [
  {
  ...
  },
  {
    ...
    angle: `Contact-centre assistants that survive Bahraini-dialect calls in ${city.name}.`,
  ...
  },
  {
    ...
    angle: 'Regulator-grade evidence files for CBB Rulebook, iGA, NHRA, NCSC, ISO 42001 and Bahrain PDPL.',
    result: 'Model cards, datasheets, bias packs and PDPL DPIA as standard.'
  }
];

export default function BahrainCityPage({ params }: { params: Params }) {
  const city = getBahrainiCityBySlug(params.slug);
  if (!city) notFound();

  const pageSlug = city.slug;
  const pagePath = `/bahrain/${city.slug}`;
  const pageUrl = `${SITE.url}${pagePath}`;

  const whatsappMessage = `Hi Brocode — I'm exploring AI delivery partners for [organisation] in ${city.name}, Bahrain. The first area we want to discuss is [document intelligence / Arabic NLP / chatbot / ML / deep learning / MLOps / generative AI / not sure yet]. Could we schedule a 30-minute call?`;

  const formConfig: PageFormConfig = {
    pageSlug,
    title: `Talk to an expert — ${city.name} desk`,
    subtitle: `One short qualification form. A senior engineer on our Bahrain delivery desk — not an SDR — responds within one business day with a named contact and a proposed shape for the engagement in ${city.name}.`,
  ...
  const relatedCities = city.relatedCitySlugs
    .map((s) => getBahrainiCityBySlug(s))
    .filter((c): c is BahrainiCity => Boolean(c));

  const relatedLinks = [
    {
      label: 'AI development in Bahrain (country overview)',
      href: '/bahrain/ai-development-in-bahrain',
      description: 'The Kingdom-wide page — CBB Rulebook, iGA, NCSC, Bahrain PDPL and the 12-week production path.'
    },
  ...
  return (
    <>
          ...
          __html: JSON.stringify([
            {
              ...
              areaServed: [
                { '@type': 'City', name: city.name, containedInPlace: { '@type': 'Country', name: 'Bahrain', identifier: 'BH' } },
            ...
            },
            {
              ...
              '@id': `${pageUrl}#webpage`,
              url: pageUrl,
              name: `AI Development Company in ${city.name}, Bahrain | Brocode`,
              inLanguage: ['en-BH', 'en-AE', 'ar-BH'],
              isPartOf: { '@id': `${SITE.url}/#website` },
              about: {
                '@type': 'Thing',
                name: `AI development services for enterprises in ${city.name}, ${city.region}, Bahrain`
              },
            ...
            },
            {
              ...
              name: `AI Development in ${city.name}, Bahrain`,
              provider: { '@id': `${SITE.url}/#organization` },
              areaServed: {
                '@type': 'City',
                name: city.name,
                containedInPlace: { '@type': 'Country', name: 'Bahrain', identifier: 'BH' }
              },
              audience: {
            ...
            },
            {
              ...
              itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: 'Bahrain', item: `${SITE.url}/bahrain/ai-development-in-bahrain` },
      ...
      <section className="bg-surface-muted py-22 md:py-30">
        <div className="container-site">
          <SectionHeading
            ...
            description={`Each tile routes to a service page with the named engineers, the reference architecture, a pre-contract benchmark offer, and the BHD rate band for ${city.name} delivery.`}
          />
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`Same model, same gates, same evidence — whether the work in ${city.name} is Arabic OCR for a Bahraini ministry, a fraud model for a CBB-supervised bank, or a deep-learning programme on a Bapco / GPIC / Alba asset.`}
            />
          </div>
      ...
      <section id="buyers-guide" className="bg-white py-22 md:py-30">
        <div className="container-site">
          <LeadMagnetGate
            ...
            contents={[
              `Procurement reality in ${city.name} and Bahrain`,
      ...
      {/* Related Bahrain cities — internal linking */}
        ...
        <section className="border-y border-ink-100 bg-surface-muted py-14">
          <div className="container-site">
            <p className="text-center text-xs font-semibold uppercase tracking-wider text-ink-500">
              Production AI delivered across Bahrain
            </p>
            ...
            <p className="mt-8 text-center text-sm text-ink-500">
              Looking for the Kingdom-wide overview?{' '}
              <Link href="/bahrain/ai-development-in-bahrain" className="text-brand underline">
                AI development in Bahrain
              </Link>
              .
            </p>
          </div>
        ...
        <div className="container-site grid grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeading
              ...
              description={`These are the questions our delivery leads field most often from ${city.name} CIO offices, sector-regulated risk teams, and Bahraini procurement reviewers.`}
            />
          </div>
          <div className="lg:col-span-7">
            <FAQ items={faqItems} />
